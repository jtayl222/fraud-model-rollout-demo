...
def load_model_and_data():
    ...
    features = [col for col in train_v2_df.columns if col.startswith('V') or col in ['Time', 'Amount']]
...
def analyze_thresholds(model, X_test, y_test):
    ...
    for threshold in test_thresholds:
        ...
        print(f"{threshold:9.1f} | {precision:9.4f} | {recall:6.4f} | {f1:8.4f} | {fp:8} | {fn:8}")
...
def plot_precision_recall_curve(precisions, recalls, thresholds):
    ...
    plt.plot(baseline_recall, baseline_precision, 'g^', markersize=10, label='Baseline (v1)')
    plt.legend()
...
def main():
    ...
    y_probs, precisions, recalls, thresholds, results = analyze_thresholds(model, X_test, y_test)
    ...
    if optimal_threshold['precision'] >= 0.95 and optimal_threshold['recall'] > 0.7351:
        print(f"✓ With threshold={optimal_threshold['threshold']}, the candidate model v2:")
        print(f"  - Maintains acceptable precision (>= 0.95)")
        print(f"  - Improves recall by {(optimal_threshold['recall']/0.7351 - 1)*100:.1f}%")
