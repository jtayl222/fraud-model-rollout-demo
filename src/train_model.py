...
def parse_args():
    ...
    parser.add_argument('--registry-name', type=str, default=None,
...
def main():
    ...
    EXPERIMENT_NAME = os.getenv("EXPERIMENT_NAME", args.experiment_name or f"fraud-detection-{MODEL_TYPE}")
    MODEL_REGISTRY_NAME = os.getenv("MODEL_REGISTRY_NAME", args.registry_name or f"fraud-detection-{MODEL_VERSION}")
    ARTIFACT_PATH = os.getenv("ARTIFACT_PATH", args.artifact_path or f"fraud-{MODEL_VERSION}-{MODEL_TYPE}")
    ...
    features = [col for col in df_train.columns if col.startswith('V') or col in ['Time', 'Amount']]
    target = 'Class'
