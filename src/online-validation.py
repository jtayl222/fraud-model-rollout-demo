...
class OnlineValidator:
    ...
    def _initialize_preprocessing(self):
        ...
        try:
            ...
            self.logger.info(
                f"Preprocessing initialized with {len(self.feature_columns)} features"
            )
    ...
    def send_test_transaction(
        self, transaction_data: Dict, model_name: str
    ) -> TransactionResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(
                    f"Transaction {transaction_id}: {model_name} predicted {prediction_prob:.4f} "
                ...
                self.logger.error(
                    f"Model {model_name} returned {response.status_code}: {response.text}"
                )
                return None
    ...
    def collect_delayed_feedback(self) -> int:
        ...
        for transaction in self.transaction_buffer:
            if (
                not transaction.feedback_received
                and transaction.actual_class is not None
    ...
    def validate_model_performance(
        self, model_name: str, lookback_minutes: int = 60
        ...
        model_transactions = [
            ...
            if (
                ...
                and t.feedback_received
                and t.timestamp >= cutoff_time
                and t.actual_class is not None
            )
        ]

        if len(model_transactions) < 10:  # Minimum sample size
            self.logger.warning(
                f"Insufficient samples for {model_name}: {len(model_transactions)}"
            )
            return None
        ...
        try:
            ...
            if len(set(y_true)) > 1:
                ...
                auc = 0.0
                self.logger.warning(
                    f"Cannot calculate AUC for {model_name}: only one class present"
                )
    ...
    def check_performance_alerts(self, result: ValidationResult) -> List[str]:
        ...
        if result.precision < PERFORMANCE_THRESHOLDS["min_precision"]:
            alerts.append(
                f"LOW PRECISION: {result.precision:.3f} < {PERFORMANCE_THRESHOLDS['min_precision']}"
            )

        if result.recall < PERFORMANCE_THRESHOLDS["min_recall"]:
            alerts.append(
                f"LOW RECALL: {result.recall:.3f} < {PERFORMANCE_THRESHOLDS['min_recall']}"
            )

        if result.f1_score < PERFORMANCE_THRESHOLDS["min_f1"]:
            alerts.append(
                f"LOW F1-SCORE: {result.f1_score:.3f} < {PERFORMANCE_THRESHOLDS['min_f1']}"
            )

        if (
            result.auc_score > 0
            and result.auc_score < PERFORMANCE_THRESHOLDS["min_auc"]
        ):
            alerts.append(
                f"LOW AUC: {result.auc_score:.3f} < {PERFORMANCE_THRESHOLDS['min_auc']}"
            )

        if result.drift_score > PERFORMANCE_THRESHOLDS["max_drift_score"]:
            alerts.append(
                f"HIGH DRIFT: {result.drift_score:.3f} > {PERFORMANCE_THRESHOLDS['max_drift_score']}"
            )

        return alerts

    def generate_test_transactions(
        self, num_transactions: int = 50, fraud_ratio: float = 0.3
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Loaded holdout data: {len(holdout_df)} transactions from Feb-Mar 2024"
            )
            fraud_count = (holdout_df["Class"] == 1).sum()
            self.logger.info(
                f"Holdout fraud rate: {fraud_count/len(holdout_df)*100:.2f}% ({fraud_count} fraud cases)"
            )
            ...
            if len(fraud_df) >= fraud_samples:
                fraud_sample = fraud_df.sample(
                    n=fraud_samples, random_state=int(time.time())
                )
                self.logger.info(
                    f"Sampled {len(fraud_sample)} fraud transactions with NEW Q1 2024 patterns"
                )
            else:
                fraud_sample = fraud_df
                self.logger.warning(
                    f"Only {len(fraud_df)} fraud cases available, using all"
                )

            # Sample normal cases
            if len(normal_df) >= normal_samples:
                normal_sample = normal_df.sample(
                    n=normal_samples, random_state=int(time.time()) + 1
                )
            else:
                normal_sample = normal_df.sample(
                    n=normal_samples, replace=True, random_state=int(time.time()) + 1
                )

            # Combine and shuffle
            combined_sample = pd.concat([fraud_sample, normal_sample]).sample(
                frac=1, random_state=int(time.time()) + 2
            )
            ...
            final_fraud_rate = sum(
                1 for t in transactions if t["actual_class"] == 1
            ) / len(transactions)
            self.logger.info(
                f"Generated {len(transactions)} test transactions with {final_fraud_rate*100:.1f}% fraud rate"
            )
            self.logger.info("These transactions contain:")
            self.logger.info(
                "  - New fraud patterns from Q1 2024 (unseen by baseline v1)"
            )
            self.logger.info(
                "  - Temporal drift that candidate v2 was trained to handle"
            )
            ...
            self.logger.error(
                f"Error generating test transactions from holdout data: {e}"
            )
            return []
    ...
    def run_validation_cycle(self, batch_size: int = 100):
        ...
        for i, transaction in enumerate(test_transactions):
            if i % 10 == 0:
                self.logger.info(
                    f"Processing transaction {i+1}/{len(test_transactions)}"
                )

            # Send to baseline model (using Seldon resource name, not MLServer internal name)
            baseline_result = self.send_test_transaction(
                transaction, "fraud-v1-baseline"
            )
            if baseline_result:
                baseline_result.feedback_received = (
                    True  # Immediate feedback for testing
                )
                baseline_results.append(baseline_result)

            # Send to candidate model (using Seldon resource name, not MLServer internal name)
            candidate_result = self.send_test_transaction(
                transaction, "fraud-v2-candidate"
            )
            if candidate_result:
                candidate_result.feedback_received = (
                    True  # Immediate feedback for testing
                )
        ...
        self.logger.info(
            f"✅ Completed inference on {len(baseline_results)} baseline + {len(candidate_results)} candidate transactions"
        )
        ...
        baseline_result = self.validate_model_performance(
            "fraud-v1-baseline", lookback_minutes=1
        )
        candidate_result = self.validate_model_performance(
            "fraud-v2-candidate", lookback_minutes=1
        )
        ...
        if baseline_result and candidate_result:
            ...
            self.logger.info(f"{'-'*55}")
            self.logger.info(
                f"{'Precision':<12} | {baseline_result.precision:<13.4f} | {candidate_result.precision:<14.4f} | {candidate_result.precision - baseline_result.precision:+12.4f}"
            )
            self.logger.info(
                f"{'Recall':<12} | {baseline_result.recall:<13.4f} | {candidate_result.recall:<14.4f} | {candidate_result.recall - baseline_result.recall:+12.4f}"
            )
            self.logger.info(
                f"{'F1-Score':<12} | {baseline_result.f1_score:<13.4f} | {candidate_result.f1_score:<14.4f} | {candidate_result.f1_score - baseline_result.f1_score:+12.4f}"
            )
            self.logger.info(
                f"{'AUC':<12} | {baseline_result.auc_score:<13.4f} | {candidate_result.auc_score:<14.4f} | {candidate_result.auc_score - baseline_result.auc_score:+12.4f}"
            )
            self.logger.info(
                f"{'Drift':<12} | {baseline_result.drift_score:<13.4f} | {candidate_result.drift_score:<14.4f} | {candidate_result.drift_score - baseline_result.drift_score:+12.4f}"
            )

            # Calculate improvement percentages
            recall_improvement = (
                (
                    (candidate_result.recall - baseline_result.recall)
                    / baseline_result.recall
                    * 100
                )
                if baseline_result.recall > 0
                else 0
            )
            precision_change = (
                (
                    (candidate_result.precision - baseline_result.precision)
                    / baseline_result.precision
                    * 100
                )
                if baseline_result.precision > 0
                else 0
            )
            ...
            self.logger.info(
                f"   Precision Change:   {precision_change:+.1f}% (v2 vs v1)"
            )
            ...
            if recall_improvement >= 5:
                ...
                self.logger.warning(
                    f"⚠️  UNEXPECTED: Candidate v2 recall improvement is only {recall_improvement:.1f}%"
                )
            ...
            self.logger.error(
                "Failed to validate performance - insufficient data or errors"
            )
    ...
    def run_continuous_validation(
        self, duration_seconds: int, interval_seconds: int = 300
    ):
        """Run continuous validation for specified duration"""
        self.logger.info(
            f"Starting continuous validation for {duration_seconds}s (interval: {interval_seconds}s)"
        )
        ...
        while time.time() - start_time < duration_seconds:
            ...
            if sleep_time > 0:
                self.logger.info(
                    f"Waiting {sleep_time:.0f}s until next validation cycle..."
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Online validation for deployed fraud detection models"
    )
    parser.add_argument(
        "--duration",
        type=int,
        default=3600,
        help="Validation duration in seconds (default: 3600)",
    )
    parser.add_argument(
        "--interval",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of test transactions per cycle (default: 50)",
    )
    parser.add_argument(
        "--single-cycle",
        action="store_true",
