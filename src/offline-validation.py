...
if not os.path.exists(csv_path):
    print(f"Error: {csv_path} not found. Please download 'creditcard.csv' from Kaggle "
...
if current_fraud_cases < required_fraud_cases:
    ...
    df_fraud_oversampled = pd.concat([df_fraud] * fraud_replication_factor, ignore_index=True)
    if len(df_fraud_oversampled) > required_fraud_cases:
        df_fraud_oversampled = df_fraud_oversampled.sample(n=required_fraud_cases, random_state=42).reset_index(drop=True)
else:
    df_fraud_oversampled = df_fraud.sample(n=required_fraud_cases, random_state=42).reset_index(drop=True)
...
if not fraud_q1_2024_indices.empty:
    ...
    if num_new_pattern_fraud > 0:
        ...
        df_q1_2024.loc[new_pattern_indices, 'Amount'] *= np.random.uniform(1.2, 1.5, size=num_new_pattern_fraud)

    df_q1_2024.loc[df_q1_2024['Class'] == 1, 'Amount'] *= 1.1

    nighttime_fraud_indices = np.random.choice(fraud_q1_2024_indices, int(0.3 * len(fraud_q1_2024_indices)), replace=False)
    for idx in nighttime_fraud_indices:
        ...
        if not (current_dt.hour >= 22 or current_dt.hour < 6):
            ...
            df_q1_2024.loc[idx, 'datetime'] = current_dt.replace(hour=random_hour, minute=random_minute, second=random_second)
...
df_holdout_test_phase2 = pd.read_csv(holdout_test_path) # Use a distinct name to avoid conflict
...
history_v1 = model_v1.fit(X_train_v1_scaled, y_train_v1,
                          ...
                          callbacks=callbacks,
                          verbose=0) # Set to 0 for less verbose output in full script
...
features = [col for col in df_train_v2.columns if col.startswith('V') or col in ['Time', 'Amount']]
target = 'Class'
...
history_v2 = model_v2.fit(X_train_v2_scaled, y_train_v2,
                          epochs=50,
                          batch_size=256,
...
print("\n--- Phase 4: Offline Validation & Deployment Decision Gate ---")
print("Step 1: Set up Environment (directories and libraries assumed from previous phases).")
...
scaler_eval.fit(X_train_v2_for_scaler_eval) # Fit scaler on the same data as model_v2 was trained
...
print(f"-----------------------------------------------------------------")
print(f"Precision    | {precision_v1_eval:.4f}        | {precision_v2_eval:.4f}        | {precision_v2_eval - precision_v1_eval:+.4f}")
print(f"Recall       | {recall_v1_eval:.4f}        | {recall_v2_eval:.4f}        | {recall_v2_eval - recall_v1_eval:+.4f}")
print(f"F1-Score     | {f1_v1_eval:.4f}        | {f1_v2_eval:.4f}        | {f1_v2_eval - f1_v1_eval:+.4f}")
print(f"AUC-ROC      | {auc_v1_eval:.4f}        | {auc_v2_eval:.4f}        | {auc_v2_eval - auc_v1_eval:+.4f}")

recall_improvement_percent = ((recall_v2_eval - recall_v1_eval) / recall_v1_eval) * 100 if recall_v1_eval != 0 else 0
...
if recall_improvement_percent >= 5 and abs(precision_v2_eval - precision_v1_eval) <= 0.01:
    print("Recommendation: **PROMOTE CANDIDATE MODEL (v2)**")
    print("Reason: Candidate model shows significant recall improvement while maintaining stable precision. This justifies proceeding to Phase 5 for A/B testing in production.")
else:
    print("Recommendation: **DO NOT PROMOTE CANDIDATE MODEL (v2) YET**")
    print("Reason: Candidate model did not meet the required performance criteria (e.g., <5% recall improvement or significant precision drop). Further investigation or retraining is needed before deployment.")
