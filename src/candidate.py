...
holdout_test_path = os.path.join(data_splits_dir, 'holdout_test.csv') # Same holdout as Phase 2
...
X_holdout_test_scaled = scaler_v2.transform(X_holdout_test) # Use same scaler for holdout transformation
...
with mlflow.start_run(run_name="fraud_v2_candidate_training"):
    ...
    mlflow.log_param("class_weight_ratio", f"{class_weight_v2[0]:.2f}:{class_weight_v2[1]:.2f}")
    ...
    mlflow.tensorflow.log_model(
        model_v2,
        artifact_path=predictable_path
    )
