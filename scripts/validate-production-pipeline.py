...
class FraudDetectionService:
    ...
    def _initialize_preprocessing(self):
        ...
        try:
            ...
            print(
                f"   Training order: {self.feature_columns[:5]}... {self.feature_columns[-2:]}"
            )
    ...
    def predict_fraud(
        self, transaction_data: Dict, model_name: str = "fraud-v2-candidate"
    ) -> Dict:
        """
        ...
        try:
            ...
            try:
                ...
                print(
                    f"   Debug: HTTP {response.status_code}, Response: {response.text[:200]}..."
                )
    ...
    def ab_test_prediction(self, transaction_data: Dict) -> Dict:
        ...
        print(
            f"   Candidate (v2): {candidate_result.get('fraud_probability', 0):.6f} "
        ...
        return {
            ...
            "comparison": {
                ...
                "both_agree": (
                    baseline_result.get("is_fraud", False)
...
def test_production_service():
    ...
    for i, transaction in enumerate(transactions):
        ...
        # Check accuracy
        baseline_correct = (
            result["baseline_result"]["is_fraud"] == transaction["actual_fraud"]
        )
        candidate_correct = (
            result["candidate_result"]["is_fraud"] == transaction["actual_fraud"]
        )
    ...
    avg_baseline_prob = np.mean(
        [r["baseline_result"]["fraud_probability"] for r in results]
    )
    avg_candidate_prob = np.mean(
