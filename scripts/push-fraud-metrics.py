...
def simulate_fraud_metrics(metrics):
    ...
    for model_name, model_metrics in models:
        ...
        metrics['accuracy'].labels(experiment=experiment, model_name=model_name).set(model_metrics['accuracy'])
        metrics['precision'].labels(experiment=experiment, model_name=model_name).set(model_metrics['precision'])
        metrics['recall'].labels(experiment=experiment, model_name=model_name).set(model_metrics['recall'])
        
        # Set traffic distribution
        metrics['traffic'].labels(experiment=experiment, model_name=model_name).set(model_metrics['traffic'])
        
        # Set business metrics
        metrics['fraud_detection_rate'].labels(experiment=experiment, model_name=model_name).set(model_metrics['fraud_detection_rate'])
        metrics['false_positive_rate'].labels(experiment=experiment, model_name=model_name).set(model_metrics['false_positive_rate'])
        
        # Simulate some request activity
        request_count = random.randint(45, 55)  # ~50 requests per model
        success_rate = 0.95 if model_name == "fraud-v1-baseline" else 0.92  # Baseline slightly more stable
        
        successful_requests = int(request_count * success_rate)
        failed_requests = request_count - successful_requests
        
        metrics['requests'].labels(experiment=experiment, model_name=model_name, status='success')._value._value = successful_requests
        metrics['requests'].labels(experiment=experiment, model_name=model_name, status='failed')._value._value = failed_requests
        
        # Simulate response times
        base_latency = 0.15 if model_name == "fraud-v1-baseline" else 0.18  # Candidate slightly slower
        for _ in range(10):  # Sample some response times
            latency = abs(np.random.normal(base_latency, 0.03))
            metrics['response_time'].labels(experiment=experiment, model_name=model_name).observe(latency)
        
        print(f"      Accuracy: {model_metrics['accuracy']:.1f}%, Precision: {model_metrics['precision']:.1f}%, Recall: {model_metrics['recall']:.1f}%")
        print(f"      Traffic: {model_metrics['traffic']:.0f}%, Requests: {request_count} ({successful_requests} success)")
...
def check_model_health():
    ...
    for model in models:
        try:
            ...
            print(f"   🔍 {model}: {'✅ Ready' if health_status[model] else '❌ Not Ready'}")
